...
bool dosis_A_detonada = false;
bool dosis_B_detonada = false;
...
void Juego::manejarEventos(Lista<Celula>* lista_celulas, Lista<Anticuerpo>* lista_anticuerpos, Lista <Suero>* lista_dosis_a, Lista <Suero>* lista_dosis_b, bool &tope, Nanobot *nanobot) {
    ...
    if((inputManager->isKeyDown(KEY_A) && !dosis_A_detonada)||entorno.dosisAExplotando())
    {
        entorno.explotarDosis(A);
        dosis_A_detonada = true;
    }

    if((inputManager->isKeyDown(KEY_B) && dosis_B_detonada)||entorno.dosisBExplotando())
    {
        entorno.explotarDosis(B);
        dosis_B_detonada = true;
    }

    if(inputManager -> isKeyDown(KEY_1) && !apretando1 && inputManager->isKeyUp(KEY_C)){
        apretando1 = true;
    ...
    if(inputManager -> isKeyDown(KEY_2) && !apretando2 && inputManager->isKeyUp(KEY_C)){
        apretando2 = true;
